function New-CIPPAlertTemplate {
    ...
    $LocationCountry = $LocationInfo.CountryOrRegion ?? $LocationInfo.countryCode ?? $LocationInfo.Country ?? $LocationInfo.country ?? 'Unknown'
    $LocationCity = $LocationInfo.City ?? $LocationInfo.city
    $LocationDisplay = if ([string]::IsNullOrWhiteSpace($LocationCity) -or $LocationCity -eq 'Unknown') {
        $LocationCountry
    } else {
    ...
    if ($InputObject -eq 'auditlog') {
        ...
        switch ($Data.Operation) {
            ...
            'UserLoggedIn' {
                ...
                $IntroText = "$($data.UserId) ($($data.Userkey)) has logged on from IP $($data.ClientIP) to the application $($Appname). According to our database this is located in $LocationDisplay. <br/><br> You have set up alerts to be notified when this happens. See the table below for more info.$Table"
                if ($ActionResults) { $IntroText = $IntroText + "<p>Based on the rule, the following actions have been taken: $($ActionResults -join '<br/>' )</p>" }
                if ($LocationInfo) {
                    $LocationTable = ($LocationInfo | ConvertTo-Html -Fragment -As List | Out-String).Replace('<table>', ' <table class="table-modern">')
                    $IntroText = $IntroText + "<p>The (potential) location information for this IP is as follows:</p>$LocationTable"
